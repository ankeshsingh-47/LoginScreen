...
import com.ysoserious.bliinder.activities.MyBliiderDates;
...
public class AndroidFoursquare extends ListActivity {
	...
	private class fourquare extends AsyncTask<View, Void, String> {
		...
		protected String doInBackground(View... urls) {
			...
			String type = "";
			if (MyBliiderDates.currentDate.getPlaceType() == PlaceType.BAR) {
				type = BAR;
			} else if (MyBliiderDates.currentDate.getPlaceType() == PlaceType.CINEMA) {
				type = CINEMA;
			} else {
				type = RESTAURANT;
			}

			// make Call to the url - ask for the place which is from a suitable
			// type.
		...
		protected void onPostExecute(String result) {
			if (temp == null) {
				...
				String[] dateLocationInfo = parseFoursquare(temp);
				MyBliiderDates.currentDate.setDateLocation(dateLocationInfo[0]);
				MyBliiderDates.currentDate.setLocation(
						Double.parseDouble(dateLocationInfo[1]),
						Double.parseDouble(dateLocationInfo[2]));
				Server.saveUpcomingDate(MyBliiderDates.currentDate);
			}
			AndroidFoursquare.activity.finish();
		}
	}
		...
		try {
			...
			if (jsonObject.has("response")) {
				if (jsonObject.getJSONObject("response").has("venues")) {
					...
					String[] nameAndCor = new String[3];
					// nameAndCor[0] = temp.get(index).getName() + ", "
					// + temp.get(index).getCategory() + ", "
					// + temp.get(index).getCity();
					nameAndCor[0] = temp.get(index).getName() + ", "
							+ temp.get(index).getCity();
					nameAndCor[1] = temp.get(index).getLatitude();
					nameAndCor[2] = temp.get(index).getLongitude();
					return nameAndCor;
				}
			}
